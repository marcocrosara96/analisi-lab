...
public class AnalyzerMemoryAccess {
    public static String program_name;
    public static String filelog_name = "memorydenied.log"; //log path
    
    //MODIFICATO RISPETTO AL 5a
    public static void main(String[] args) {  
        ...
        ArrayList<String> usefulLog = logIntegration(logFilter(result, "SIGSEGV"));
        if(usefulLog.size() != 0){
    ...
    public static ArrayList<String> logIntegration(ArrayList<String> logs){
        ...
        for (String log : logs){
            if(log.contains("si_addr=0x")){ //controllo che il SIGSEGV corrisponda a un errore di accesso a un indirizzo di memoria
                log = getTimestamp() + "\t" + program_name + "\t" + extractMemoryAddressFromString(log) + "\t" + log;
                new_logs.add(log);
            }
        }
        return new_logs;
    ...
    /**
     * Estrae da una stringa la porzione che rappresenta un indirizzo di memoria del tipo 0x...
     * es. SIGSEGV {si_signo=SIGSEGV, si_code=SEGV_MAPERR, si_addr=0x6708d820} ---> la funzione ritorna 0x6708d820
     * @param s stringa su cui effettuare l'operazione di estrazione
     * @return stringa che in s è l'indirizzo di memoria 0x...
     */
    public static String extractMemoryAddressFromString(String s){
        try{
            return s.substring(s.indexOf("0x"), s.lastIndexOf("}"));
        }catch(Exception e){
            return "";
        }
    }
